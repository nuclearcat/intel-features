...
pub const FEATURES: &[FeatureDef] = &[
    ...
    defk("sse", "SSE", Isa, "128-bit single-precision SIMD (ADDPS, MULPS, MOVAPS)", "sse"),
    defk("sse2", "SSE2", Isa, "128-bit double-precision and integer SIMD; x86-64 baseline", "sse2"),
    defk("sse3", "SSE3", Isa, "Horizontal/add-subtract SIMD and unaligned loads (HADDPS, LDDQU)", "pni"),
    defk("ssse3", "SSSE3", Isa, "Byte shuffle, sign, align, and horizontal integer operations", "ssse3"),
    defk("sse4_1", "SSE4.1", Isa, "Blend, dot-product, rounding, and wider integer SIMD", "sse4_1"),
    defk("sse4_2", "SSE4.2", Isa, "String comparison, text processing, and CRC32", "sse4_2"),
    defk("avx", "AVX", Isa, "256-bit floating-point SIMD with three-operand instructions", "avx"),
    defk("avx2", "AVX2", Isa, "256-bit integer SIMD, gathers, shifts, and permutations", "avx2"),
    defk("fma", "FMA3", Isa, "Fused multiply-add for faster, more accurate vector math", "fma"),
    defk("f16c", "F16C", Isa, "Convert between IEEE FP16 and FP32 (VCVTPH2PS/VCVTPS2PH)", "f16c"),
    // AVX-512 family
    ...
    defk("avx_vnni", "AVX-VNNI", Isa, "128/256-bit integer dot products for inference and DSP", "avx_vnni"),
    def("avx_vnni_int8", "AVX-VNNI-INT8", Isa, "Signed INT8 vector dot-product instructions"),
    def("avx_vnni_int16", "AVX-VNNI-INT16", Isa, "Signed INT16 vector dot-product instructions"),
    def("avx_ifma", "AVX-IFMA", Isa, "52-bit integer multiply-add for big-number arithmetic"),
    def("avx_ne_convert", "AVX-NE-CONVERT", Isa, "Low-overhead BF16/FP16 conversion without exceptions"),
    // AMX
    ...
    // Bit-manip / integer
    defk("popcnt", "POPCNT", Isa, "Count set bits in an integer", "popcnt"),
    defk("lzcnt", "LZCNT/ABM", Isa, "Count leading zero bits; useful for normalization and bit scans", "abm"),
    defk("bmi1", "BMI1", Isa, "Bit extraction and masking (ANDN, BEXTR, BLSI/BLSR)", "bmi1"),
    defk("bmi2", "BMI2", Isa, "Bit deposit/extract and multiply-without-flags (PDEP, PEXT, MULX)", "bmi2"),
    defk("adx", "ADX", Isa, "Independent carry chains for big integers (ADCX, ADOX)", "adx"),
    defk("movbe", "MOVBE", Isa, "Load/store with byte reversal for big-endian data", "movbe"),
    // Crypto
    defk("aes", "AES-NI", Isa, "AES encryption/decryption rounds and key generation", "aes"),
    defk("vaes", "VAES", Isa, "Vector AES rounds across multiple 128-bit blocks", "vaes"),
    defk("pclmulqdq", "PCLMULQDQ", Isa, "Carry-less multiply for GCM, CRC, and finite-field crypto", "pclmulqdq"),
    defk("vpclmulqdq", "VPCLMULQDQ", Isa, "Vector carry-less multiply for parallel GCM/CRC", "vpclmulqdq"),
    defk("sha", "SHA-NI", Isa, "Hardware SHA-1 and SHA-256 message/round operations", "sha_ni"),
    defk("gfni", "GFNI", Isa, "Galois-field affine transforms and multiplication", "gfni"),
    // RNG
    defk("rdrand", "RDRAND", Isa, "Random values from the CPU deterministic random-bit generator", "rdrand"),
    defk("rdseed", "RDSEED", Isa, "Seed-quality entropy for initializing software random generators", "rdseed"),
    // TSX
    defk("rtm", "RTM (TSX)", Isa, "Restricted transactional memory", "rtm"),
    defk("hle", "HLE (TSX)", Isa, "Hardware lock elision (deprecated)", "hle"),
    // Memory/streaming
    defk("clflushopt", "CLFLUSHOPT", Isa, "Ordered, optimized cache-line eviction for persistence and DMA", "clflushopt"),
    defk("clwb", "CLWB", Isa, "Write back a cache line without necessarily invalidating it", "clwb"),
    defk("movdiri", "MOVDIRI", Isa, "Direct 32/64-bit stores for MMIO and device queues", "movdiri"),
    defk("movdir64b", "MOVDIR64B", Isa, "Atomic 64-byte direct stores for devices and accelerators", "movdir64b"),
    defk("serialize", "SERIALIZE", Isa, "Fully order prior and later instruction execution", "serialize"),
    defk("waitpkg", "WAITPKG", Isa, "Low-power monitored waits and timed pauses (UMONITOR/UMWAIT/TPAUSE)", "waitpkg"),
    def("hreset", "HRESET", Isa, "Reset selected CPU execution-history state, including Thread Director history"),
    def("prefetchi", "PREFETCHI", Isa, "Code prefetch to L2"),
    defk("fsrm", "FSRM", Isa, "Fast short copies and fills using REP MOVSB/STOSB", "fsrm"),
    defk("rdpid", "RDPID", Isa, "Read a kernel-configured per-CPU identifier without serialization", "rdpid"),
    defk("cmpxchg16b", "CMPXCHG16B", Isa, "Atomic 128-bit compare-and-exchange", "cx16"),
    defk("fsgsbase", "FSGSBASE", Isa, "Read/write FS and GS bases directly from user mode", "fsgsbase"),
    // XSAVE family
    defk("xsave", "XSAVE", Isa, "Save/restore selected extended CPU state such as AVX registers", "xsave"),
    defk("xsaveopt", "XSAVEOPT", Isa, "Skip unchanged state while saving an XSAVE area", "xsaveopt"),
    defk("xsavec", "XSAVEC", Isa, "Save extended state in a compacted layout", "xsavec"),
    defk("xsaves", "XSAVES", Isa, "Save user and supervisor extended state in compact form", "xsaves"),
    // ================= Security ======================================================
    ...
    defv("igpu", "Integrated GPU", Accelerators, "Intel graphics (i915/xe)"),
    defv("npu", "NPU / VPU", Accelerators, "Neural/AI accelerator (intel_vpu)"),
    defv("gna", "GNA", Accelerators, "Gaussian & Neural Accelerator"),
    defv("dsa", "DSA", Accelerators, "Data Streaming Accelerator (idxd)"),
    defv("iaa", "IAA", Accelerators, "In-Memory Analytics Accelerator (idxd)"),
    defv("qat", "QAT", Accelerators, "QuickAssist crypto/compression"),
    defv("dlb", "DLB", Accelerators, "Dynamic Load Balancer"),
    // ================= Chipset & Platform Devices ====================================
    defv("pch", "PCH / Chipset", Platform, "Platform Controller Hub (LPC/eSPI bridge)"),
    defv("csme", "ME / CSME", Platform, "Management Engine (MEI interface)"),
    defv("ethernet", "Ethernet", Platform, "Integrated Intel NIC"),
    defv("wifi", "Wi-Fi", Platform, "Intel wireless (CNVi/discrete)"),
    defv("bluetooth", "Bluetooth", Platform, "Bluetooth controller"),
    defv("audio", "HD Audio / SST", Platform, "Audio DSP / codec"),
    defv("smbus", "SMBus", Platform, "SMBus/i2c host controller"),
    defv("spi_flash", "SPI Flash Controller", Platform, "BIOS/SPI flash controller"),
